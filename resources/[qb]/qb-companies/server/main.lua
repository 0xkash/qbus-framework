...
Citizen.CreateThread(function()
    QBCore.Functions.ExecuteSql("SELECT * FROM `companies`", function(result)
            ...
            if companyName ~= "" then
                if IsNameAvailable(companyName) then
                    if cashAmount >= Config.CompanyPrice then
                        ...
                        QBCore.Functions.ExecuteSql("INSERT INTO `companies` (`name`, `label`, `owner`) VALUES ('"..companyName.."', '"..companyLabel.."', '"..Player.PlayerData.citizenid.."')")
...
AddEventHandler('qb-companies:server:removeCompany', function(companyName)
    ...
    if companyName ~= nil then 
        if IsCompanyOwner(companyName, Player.PlayerData.citizenid) then
            ...
            QBCore.Functions.ExecuteSql("DELETE FROM `companies` WHERE `name` = '"..escapeSqli(companyName).."'")
...
AddEventHandler('qb-companies:server:quitCompany', function(companyName)
    ...
    if companyName ~= nil then 
        if IsEmployee(companyName, Player.PlayerData.citizenid) then
            ...
            QBCore.Functions.ExecuteSql("UPDATE `companies` SET `employees` = '"..json.encode(updatedEmployees).."' WHERE `name` = '"..escapeSqli(companyName).."'")
        else
            TriggerClientEvent('QBCore:Notify', src, 'Je bent geen eigenaar van dit bedrijf..', 'error', 4000)
        end
    else
    ...
    if OtherPlayer ~= nil then 
        if IsCompanyOwner(companyName, Player.PlayerData.citizenid) then
            if rank > 0 and rank < Config.MaxRank then
                ...
                QBCore.Functions.ExecuteSql("UPDATE `companies` SET `employees` = '"..json.encode(Config.Companies[companyName].employees).."' WHERE `name` = '"..escapeSqli(companyName).."'")
                TriggerClientEvent("qb-companies:client:setCompanies", -1, Config.Companies)
...
function IsNameAvailable(name)
    local retval = false
    QBCore.Functions.ExecuteSql("SELECT * FROM `companies` WHERE `name` = '"..name.."'", function(result)
        print(result[1])
        if result[1] ~= nil then 
            retval = false
        else
