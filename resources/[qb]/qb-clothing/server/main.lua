...
AddEventHandler('qb-clothing:saveSkin', function(model, skin)
    ...
    if model ~= nil and skin ~= nil then 
		QBCore.Functions.ExecuteSql(true, "DELETE FROM `playerskins` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'")
        Citizen.Wait(500)
        QBCore.Functions.ExecuteSql(true, "INSERT INTO `playerskins` (`citizenid`, `model`, `skin`, `active`) VALUES ('"..Player.PlayerData.citizenid.."', '"..model.."', '"..skin.."', 1)")
    end
end)
...
AddEventHandler('qb-clothes:loadPlayerSkin', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    QBCore.Functions.ExecuteSql(true, "SELECT * FROM `playerskins` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' AND `active` = 1", function(result)
...
AddEventHandler("qb-clothes:saveOutfit", function(outfitName, model, skinData)
    ...
    if model ~= nil and skinData ~= nil then
        ...
        QBCore.Functions.ExecuteSql(true, "INSERT INTO `player_outfits` (`citizenid`, `outfitname`, `model`, `skin`, `outfitId`) VALUES ('"..Player.PlayerData.citizenid.."', '"..outfitName.."', '"..model.."', '"..json.encode(skinData).."', '"..outfitId.."')")
...
AddEventHandler("qb-clothing:server:removeOutfit", function(outfitName, outfitId)
    ...
    QBCore.Functions.ExecuteSql(true, "DELETE FROM `player_outfits` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' AND `outfitname` = '"..outfitName.."' AND `outfitId` = '"..outfitId.."'")
    SetTimeout(100, function()
        QBCore.Functions.ExecuteSql(true, "SELECT * FROM `player_outfits` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(result)
            if result[1] ~= nil then
                TriggerClientEvent('qb-clothing:client:reloadOutfits', src, result)
...
QBCore.Commands.Add("helm", "Zet je helm/pet/hoed op of af..", {}, false, function(source, args)
    TriggerClientEvent("qb-clothing:client:adjustfacewear", source, 1) -- Hat
end)

QBCore.Commands.Add("bril", "Zet je bril op of af..", {}, false, function(source, args)
